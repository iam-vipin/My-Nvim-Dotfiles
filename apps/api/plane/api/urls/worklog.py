...
# Module imports
from plane.api.views import WorkItemWorklogEndpoint, ProjectWorklogAPIEndpoint
...
old_url_patterns = [
    ...
    path(
        ...
        WorkItemWorklogEndpoint.as_view(http_method_names=["post", "get"]),
        name="worklogs",
    ),
    # Deprecated url path
    path(
        "workspaces/<str:slug>/projects/<uuid:project_id>/issues/<uuid:issue_id>/worklogs/<uuid:pk>/",
        WorkItemWorklogEndpoint.as_view(http_method_names=["patch", "delete"]),
        name="worklogs",
    ),
    path(
