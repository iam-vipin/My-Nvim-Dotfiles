...
def is_project_intakes_enabled(workspace_slug: str, project_id: str, raise_exception: bool = True):
    try:
        project = Project.objects.filter(workspace__slug=workspace_slug, id=project_id).first()
...
@sync_to_async
def is_project_intakes_enabled_async(workspace_slug: str, project_id: str, raise_exception: bool = True):
...
def get_intake_stats(workspace_slug: str, project_id: str, intake_work_item: str):
    ...
    relations_count = IssueRelation.objects.filter(Q(issue_id=workitem_id) | Q(related_issue_id=workitem_id)).count()
...
def get_intake(workspace_slug: str, project_id: str):
    try:
        project_intake = Intake.objects.filter(workspace__slug=workspace_slug, project_id=project_id).first()
...
def intake_work_item_base_query(workspace_slug: str, project_id: str, user_id: Optional[str] = None):
    try:
        intake = get_intake(workspace_slug=workspace_slug, project_id=project_id)
        intake_id = intake.id
        ...
        if user_id:
            intake_work_item_base_query = intake_work_item_base_query.filter(created_by_id=user_id)
...
def intake_work_item_base_query_async(workspace_slug: str, project_id: str):
    return intake_work_item_base_query(workspace_slug=workspace_slug, project_id=project_id)
...
def get_intake_work_items(
    ...
    base_query = intake_work_item_base_query(workspace_slug=workspace_slug, project_id=project_id, user_id=user_id)

    if is_snoozed_work_items_required:
        base_query = base_query.filter(
            (Q(snoozed_till__gte=timezone.now()) & Q(status=IntakeWorkItemStatusType.SNOOZED.value))
