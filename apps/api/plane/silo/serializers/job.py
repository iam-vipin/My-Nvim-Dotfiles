...
class ImportJobAPISerializer(BaseSerializer):
    ...
    initiator_id = serializers.PrimaryKeyRelatedField(source="initiator", queryset=User.objects.all())
    report_id = serializers.PrimaryKeyRelatedField(source="report", queryset=ImportReport.objects.all())
    ...
    def validate_relation_map(self, value):
        ...
        if not isinstance(value, dict) or "issue" not in value:
            raise serializers.ValidationError("relation_map must contain an 'issue' key")

        valid_relations = set(x[0] for x in IssueRelationChoices.choices) | {"parent_id"}

        for issue_id, relation_entries in value["issue"].items():
            if not isinstance(relation_entries, list):
                raise serializers.ValidationError(f"Relations for issue {issue_id} must be a list")

            for relation_entry in relation_entries:
                if not isinstance(relation_entry, dict):
                    raise serializers.ValidationError("Each relation must be a dictionary")

                if "identifier" not in relation_entry or "relation" not in relation_entry:
                    raise serializers.ValidationError("Each relation must have 'identifier' and 'relation' keys")

                if relation_entry["relation"] not in valid_relations:
                    raise serializers.ValidationError(f"Invalid relation type: {relation_entry['relation']}")
    ...
    def update(self, instance, validated_data):
        ...
        if self.partial:
            ...
            merged_relation_map = dict(instance.relation_map) if instance.relation_map else {}
            ...
            if "issue" in new_relation_map:
                for issue_id, new_relations in new_relation_map["issue"].items():
                    if issue_id in merged_relation_map["issue"]:
                        existing_relations = {
                            (rel["identifier"], rel["relation"]) for rel in merged_relation_map["issue"][issue_id]
                        }
                        for new_rel in new_relations:
                            existing_relations.add((new_rel["identifier"], new_rel["relation"]))
