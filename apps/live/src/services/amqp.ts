...
const makeAmqpService = Effect.gen(function* () {
  ...
  const exchange = config.EVENT_STREAM_EXCHANGE;
  const prefetchCount = config.PREFETCH_COUNT;
  const maxRetries = 5;
...
export class AmqpService extends Effect.Service<AmqpService>()("@live/AmqpService", {
