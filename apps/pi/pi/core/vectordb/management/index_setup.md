...
### Issues Pipeline (Smart conditional embedding)

```http
PUT /_ingest/pipeline/issue-embedding-pipeline
{
  "description": "Embed issues (requires name & description ≥3 chars each)",
  "processors": [
    {
      "ml_inference": {
    ...
    {
      "script": {
        "source": "String n = ctx.name == null ? '' : ctx.name.trim(); String d = ctx.description == null ? '' : ctx.description.trim(); if (n.length() >= 3 && d.length() >= 3) { ctx.content = n + ' ' + d; }",
        "if": "ctx.name != null && ctx.description != null && ctx.name.trim().length() >= 3 && ctx.description.trim().length() >= 3"
      }
    },
...
### Pages Pipeline (Simple embedding)

```http
PUT /_ingest/pipeline/pages-embedding-pipeline
{
  "description": "Simple embedding for pages",
  "processors": [
    {
      "ml_inference": {
        "model_id": "YOUR_ML_MODEL_ID",
