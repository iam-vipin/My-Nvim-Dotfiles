...
import type { Client, ExIssue, ExModule } from "@plane/sdk";
...
export class LinearIssueMentionParserExtension implements IParserExtension {
  ...
  async getIssue(externalIssueId: string): Promise<ExIssue | null> {
    ...
    const planeIssue = await this.config.planeClient.issue.getIssueWithExternalId(
      this.config.workspaceSlug,
      this.config.projectId,
      issue.id,
      E_IMPORTER_KEYS.LINEAR
    );
    if (!planeIssue) {
      return null;
    }
...
export class LinearProjectMentionParserExtension implements IParserExtension {
  ...
  async getPlaneModule(projectMentionIdentifier: string): Promise<ExModule | null> {
    ...
    const planeModule = await this.config.planeClient.modules.getModuleByExternalId(
      this.config.workspaceSlug,
      this.config.projectId,
      project.id,
      E_IMPORTER_KEYS.LINEAR
  ...
  */
  async createMentionComponent(node: HTMLElement, planeModule: ExModule): Promise<HTMLElement> {
    ...
    node.setAttribute(
      "href",
      `${this.config.APP_BASE_URL}/${this.config.workspaceSlug}/projects/${this.config.projectId}/modules/${planeModule.id}`
    );
