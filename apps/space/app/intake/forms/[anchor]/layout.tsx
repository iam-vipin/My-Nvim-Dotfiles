...
const DEFAULT_DESCRIPTION = "Made with Plane, an AI-powered work management platform.";

interface TypeFormMetadata {
  name?: string;
  description?: string;
  cover_image?: string;
}
...
export async function loader({ params }: Route.LoaderArgs) {
  ...
  try {
    const response = await fetch(`${process.env.VITE_API_BASE_URL}/api/public/anchor/${anchor}/intake/form/settings/`);

    if (!response.ok) {
      return { metadata: null };
    }

    const metadata: TypeFormMetadata = await response.json();
    return { metadata };
  } catch (error) {
    console.error("Error fetching type form metadata:", error);
    return { metadata: null };
  }
}
...
function TypeFormLayout(props: Route.ComponentProps) {
  const { anchor } = props.params;
  // store hooks
  const { fetchTypeFormSettings, settings } = useIntake();

  const { error } = useSWR(
    anchor ? `TYPE_FORM_SETTINGS_${anchor}` : null,
    anchor ? () => fetchTypeFormSettings(anchor) : null,
...
export default observer(TypeFormLayout);
