...
export const ModuleTimelineBlock = observer(function ModuleTimelineBlock(props: Props) {
...
export const ModuleTimelineSidebarBlock = observer(function ModuleTimelineSidebarBlock(props: Props) {
  const { moduleId } = props;
  const { workspaceSlug } = useParams();
  // store hooks
  const { getModuleById } = useModule();
