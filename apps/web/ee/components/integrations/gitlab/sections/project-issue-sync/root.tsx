...
} from "@/plane-web/components/integrations/gitlab/sections/project-issue-sync";
//  plane web hooks
...
export const ProjectIssueSyncRoot = observer(function ProjectIssueSyncRoot({
  ...
  // derived values
  const entityConnectionMap = entityConnectionIds.map((id) => entityConnectionById(id));
  ...
  const entityConnection = entityConnectionMap.reduce(
    (result: { [key: string]: TGitlabEntityConnection[] }, entity) => {
      if (entity?.type !== EGitlabEntityConnectionType.PROJECT_ISSUE_SYNC) return result;
    ...
    <div className="relative border border-subtle rounded">
      ...
      {Object.keys(entityConnection).length > 0 ? (
        ...
        <div className="p-10 relative text-center">
          <div className="text-body-xs-regular text-secondary">
            {t("gitlab_integration.project_issue_sync_empty_state")}
          </div>
        </div>
      )}
