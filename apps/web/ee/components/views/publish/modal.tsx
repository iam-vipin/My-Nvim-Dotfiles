...
import type { IProjectView, TPublishViewSettings } from "@plane/types";
...
export const PublishViewModal: React.FC<Props> = observer((props) => {
  ...
  const handleUpdatePublishSettings = async (payload: Partial<TPublishViewSettings>) => {
    ...
    await updatePublishedView(workspaceSlug.toString(), view.project, view.id, payload).then((res) => {
      setToast({
  ...
  const handleUnPublishView = async () => {
    ...
    await unPublishView(workspaceSlug.toString(), view.project, view.id)
      .catch(() =>
        setToast({
