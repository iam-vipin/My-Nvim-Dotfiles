  ...
  const columns = [
    {
      ...
      tdRender: (rowData: RowData) => {
        if (!rowData.initiated_by) return <div>-</div>;
        const userDetails = getUserDetails(rowData.initiated_by);
        if (!userDetails) return <div>-</div>;
        const { avatar_url, display_name, email } = userDetails;
        return (
          ...
          <>
            {rowData.status == "completed" ? (
              <a target="_blank" href={rowData?.url} rel="noopener noreferrer">
                <button
                  className="w-full flex items-center gap-1 text-accent-primary font-medium"
