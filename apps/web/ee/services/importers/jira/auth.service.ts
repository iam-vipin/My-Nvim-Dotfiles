...
export class JiraAuthService {
  ...
  async jiraAuthVerification(workspaceId: string, userId: string): Promise<TServiceAuthConfiguration | undefined> {
    return this.axiosInstance
      .get(`/api/credentials/${workspaceId}/${userId}/?source=${E_IMPORTER_KEYS.JIRA}`)
      .then((res) => res.data)
      .catch((error) => {
  ...
  async jiraApiTokenVerification(
    ...
    return this.axiosInstance
      .post(
        `/api/credentials/${workspaceId}/${userId}/token-verify/?source=${E_IMPORTER_KEYS.JIRA}&token=${externalApiToken}`
      )
      .then((res) => res.data)
      .catch((error) => {
        throw error?.response?.data;
  ...
  async jiraAuthDeactivate(workspaceId: string, userId: string): Promise<void | undefined> {
    return this.axiosInstance
      .post(`/api/credentials/${workspaceId}/${userId}/deactivate/?source=${E_IMPORTER_KEYS.JIRA}`)
      .then((res) => res.data)
      .catch((error) => {
        throw error?.response?.data;
      });
